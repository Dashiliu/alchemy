...
public class KafkaConnectorDescriptor implements ConnectorDescriptor {
    ...
    private void createDeserialization(TypeInformation<Row> returnType, FormatDescriptor format, AlchemyKafkaTableSource.Builder tableSourceBuilder) throws ClassNotFoundException {
        DeserializationSchema<Row> deserializationSchema=null;
        switch (format.getType()){
            case Constants.TYPE_VALUE_FORMAT_HESSIAN:
                deserializationSchema=new HessianRowDeserializationSchema(returnType,Class.forName(format.getClassName()));
                break;
            case Constants.TYPE_VALUE_FORMAT_PB:
                deserializationSchema=new ProtostuffRowDeserializationSchema(returnType,Class.forName(format.getClassName()));
                break;
            default:
        }
    ...
    private <T> TypeInformation<Row> createSchema(List<Field> schema, T t)
