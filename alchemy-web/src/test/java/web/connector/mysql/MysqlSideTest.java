...
public class MysqlSideTest extends BaseCluster {
    ...
    public void simple_mysql() throws Exception {
        Response response = execute("select c.first, c.id, c.score ,c.last ,s.name  from csv_table_test as c join side_simple as s on c.id = s.id");
        assert response.isSuccess();
    }

    @Test
    public void nest_mysql() throws Exception {
        Response response = execute("select * from (select c.first, c.id, c.score ,c.last ,s.name  from csv_table_test as c join side_simple as s on c.id = s.id where c.score > 1 and s.name like 'z%') as d ");
        assert response.isSuccess();
    }
